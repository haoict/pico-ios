<template>
  <div class="flex flex-col gap-6 mb-8 px-2 !relative !z-[100] !pointer-events-auto">
    <!-- title & actions -->
    <div class="flex justify-between items-center mb-1">
      <div class="flex flex-col">
        ...
        <span class="text-xs font-medium text-white/40 tracking-wider uppercase mt-1">
          {{ gamesCount }} Cartridges
        </span>
      </div>
    ...
    <div class="flex gap-3">
      <div class="relative flex-1 group">
        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
          <svg class="h-4 w-4 text-white/40 group-focus-within:text-purple-400 transition-colors"
      ...
      <div class="relative min-w-[140px] z-[200]">
        <button @click="$emit('update:sortDropdownOpen', !sortDropdownOpen)"
        ...
        <transition name="fade">
          <div v-if="sortDropdownOpen"
            ...
            <div v-for="opt in sortOptions" :key="opt.value"
              class="px-4 py-3 text-sm cursor-pointer transition-colors border-b border-white/5 last:border-0" :class="sortBy === opt.value
      ...
      <button @click="$emit('update:gridSize', cycleGridSizeValue())"
...
const cycleGridSizeValue = () => {
  const modes = ['S', 'M', 'L'];
  const currentIndex = modes.indexOf(props.gridSize);
