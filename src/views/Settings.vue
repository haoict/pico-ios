<template>
  <div class="min-h-screen w-full bg-oled-dark text-white p-6 pt-16 overflow-y-auto no-scrollbar">
    ...
    <div class="space-y-8 max-w-2xl mx-auto">
      <section>
        <div class="space-y-3">
          <div v-for="(item, index) in settingsItems" :key="item.id" :ref="(el) => setItemRef(el, index)"
            ...
            <!-- virtual joystick -->
            <div v-if="item.type === 'toggle'" class="w-12 h-7 rounded-full transition-colors relative"
      ...
      <!-- Danger Zone -->
      <section>
        <h2 class="text-xs font-bold text-red-500 uppercase tracking-widest mb-3 ml-1">
          Danger Zone
        </h2>
        <div class="space-y-3">
          <div v-for="(item, index) in dangerItems" :key="item.id" @click="item.action"
